...
def format_table_row(network: WiFiNetwork, index: int) -> str:
    ...
    quality_bar = "█" * (quality // SIGNAL_BAR_SEGMENTS) + "░" * (
        SIGNAL_BAR_SEGMENTS - quality // SIGNAL_BAR_SEGMENTS
    )
    security_icon = "🔒" if network.is_secured() else "🔓"

    return (
        f"{index:2d} │ {ssid:<20} │ {bssid:<17} │ {quality_bar} {quality:3d}% │ "
...
def print_score(
    network: WiFiNetwork, score: int, reasons: list[str], risk_level: str, rating: int
):
...
async def find_network(
    networks: list[WiFiNetwork], ssid: str | None, bssid: str | None
) -> WiFiNetwork | None:
    if ssid and bssid:
        return next(
            (n for n in networks if n.get_ssid() == ssid and n.get_bssid() == bssid),
            None,
        )
    elif ssid:
...
def main():
    parser = argparse.ArgumentParser(
        prog="wst", description="WiFi network scanner and analyzer"
    )
